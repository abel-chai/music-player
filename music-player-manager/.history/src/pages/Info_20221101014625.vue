  ...
  export default{
    ...
    methods:{
      draw(){
        ...
        this.$axios({
          ...
          response => {
            if(response.data.type == "success"){
              ...
              // for(i = 0; i < this.sheetOption.series.data.length; i++){
              //   for(j = 0; j < sheetStyle.length; j++){
              //     if(this.sheetOption.series.data[i].name === sheetStyle[j].){
              //       this.sheetOption.series.data[j].name = response.data.data.get(sheetStyle[i])
              //     }
              //   }
              // }
              for(let i = 0; i < this.sheetOption.series.data.length; i++){
                temp = this.sheetOption.series.data[i].name;
                this.sheetOption.series.data[i].value = sheetStyle.temp
                console.log(sheetStyle.temp)
                console.log(this.sheetOption.series.data[i].value)
              }
            }else{
              this.$message.error(response.data.message);
            }
        ...
        //   response => {
        //     if(response.data.type == "success"){
        //       const singerCountry = Object.keys(response.data.data)
        //       for(var i = 0; i < singerCountry.length; i++){
        //         for(var j = 0; j < this.singerOption.xAxis.data.length; j++){
        //           if(this.singerOption.xAxis.data[j] === singerCountry[i]){
        //             this.singerOption.series[0].data[j] = 1
        //             break;
        //           }
        //         }
        //       }
