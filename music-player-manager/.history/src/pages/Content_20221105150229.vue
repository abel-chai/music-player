<template>
  <div>
    ...
    <el-dialog title="添加歌曲" :visible.sync="addVisible">
      <el-form label-width="80px" ref="":model="form">
        <el-form-item 
          prop="singerName" 
          label="歌手名字"
  ...
  export default {
    ...
    methods: {
      ...
      handleSearch() {
        ...
        this.$axios({
          method: 'get',
          url: `/sheetContain/search/withSinger/${this.$route.query.sheetId}/${this.query.songName}`,
          headers: {'token': this.token}
        }).then(
          response => {
            if(response.data.type == "success"){
              const songSinger = response.data.data
              let tempData = []
              for(var i =0; i < songSinger.length; i++){
                const songSingerName = songSinger[i].singer.singerName + '-' + songSinger[i].song.songName
                const songSingerId = songSinger[i].song.id
