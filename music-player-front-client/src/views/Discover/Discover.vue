            ...
            <ul class="new-songs">
                <li v-for="(item,index) in newSongs" :key="index">
                    ...
                    <div class="music-info">
                        <p class="music-name">{{item.song.songName}}</p>
                        <p class="music-singer" style="display:inline;margin:0 5px">{{item.singerName}}</p>
                    </div>
                </li>                                                                                                                       
            </ul>
        </div>
...
<script>
import { bannerAPI,recommendSonglistAPI,recommendSongAPI,playMusicAPI } from '@/utils/api'
...
export default {
    ...
    created(){
        ...
        // 获取推荐歌单
        recommendSonglistAPI('').then(res=>{
            // console.log(res)
            this.recommendList = res.data.data.slice(0, 10)
        })
    ...
    methods:{
        ...
        playMusic(item){
            playMusicAPI(item.song.id).then(res=>{
                if(res.data.data.url){
                    this.songUrl = res.data.data.url

                    let musicInfo = {
                        imgUrl:item.song.img,
                        artistInfo:item.singerName,
